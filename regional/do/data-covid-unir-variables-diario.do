...
* 1.3 Casos positivos antigenica por dia
use "${data}\data-covid-unir-variables.dta", clear

keep if positivo_antigenica == 1

collapse (count) var_id, by(fecha_resultado)

tsset fecha_resultado, daily
tsfill
generate total_positivo_antigenica=sum(var_id)

rename var_id positivos_antigenica

save "${regional}/data/data-covid-unir-variables-positivo-antigenica.dta", replace
...
keep if prueba_antigenica == 1

collapse (count) var_id, by(fecha_resultado)

tsset fecha_resultado, daily
tsfill
generate total_muestra_antigenica =sum( var_id )

rename var_id muestra_antigenica 

save "${regional}/data/data-covid-unir-variables-muestra-antigenica.dta", replace
...
* 6.3 Inicio de síntoma prueba antigenica
use "${data}\data-covid-unir-variables.dta", clear
...
tsset fecha_inicio, daily
tsfill
generate total_inicio_antigenica=sum(var_id)

rename fecha_inicio fecha_resultado
rename var_id inicio_antigenica

save "${regional}/data/data-inicio-antigenica", replace
********************************************************************************
...
drop _merge
merge 1:1 fecha_resultado using "${regional}\data\data-covid-unir-variables-positivo-antigenica"
drop _merge  
...
drop _merge
merge 1:1 fecha_resultado using "${regional}\data\data-covid-unir-variables-muestra-antigenica"
drop _merge
...
drop _merge 
merge 1:1 fecha_resultado using "${regional}\data\data-inicio-antigenica"
drop _merge 
